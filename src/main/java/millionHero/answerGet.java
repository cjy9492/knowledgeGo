...
public class answerGet {
    ...
    public static List getanswaer(String question, List answers) throws Exception {
        ...
        if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK)
        {
            ...
            if(!"出题后，此处自动显示题目！".equals(qustion.get(1).text())){
            ...
            Elements answerc = doc.getElementsByClass("answer-content answer-c");
                ...
                String[] tqustions=tqustion.split("\\.");
          ...
          if (!tqustions[1].equals(lastQues)) {
                lastQues = tqustions[1];
                System.out.println(tqustions[1].replace(" ",""));
                quest.add(tqustions[1].replace(" ",""));
                quest.add(answer);
            }
            } else {
                quest.add("no");
